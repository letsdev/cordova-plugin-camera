    ...
    switch (options.encodingType) {
        ...
        case EncodingTypeJPEG:
        {
            ...
            if (options.usesGeolocation) {
                //FK 22.06.22: due to https://github.com/apache/cordova-plugin-camera/issues/778 we are using cordova-camera plugin as a local plugin with adjustments to fix camera picker behavior
                //since UIImagePickerControllerMediaMetadata is only available for images captured by the camera (and not picked from the image library, see https://developer.apple.com/documentation/uikit/uiimagepickercontrollermediametadata?language=objc)
                //we are also using the picked image when no exif metadata can be read
                self.data = data;
...
- (void)locationManager:(CLLocationManager*)manager didUpdateToLocation:(CLLocation*)newLocation fromLocation:(CLLocation*)oldLocation
{
    ...
    if ([motionManager isDeviceMotionAvailable]) {
        ...
        [motionManager startDeviceMotionUpdatesUsingReferenceFrame:CMAttitudeReferenceFrameXTrueNorthZVertical
            ...
            [GPSDictionary setObject:@(motionData.heading) forKey:(NSString *)kCGImagePropertyGPSImgDirection];
